...
void hexdump_kern_space(vm_address_t address, size_t size) {
	unsigned char* buf = malloc(size);
    read_kernel(address, size, buf);
    hexdump(buf, size);
    free(buf);
}
...
void print_usage() {
	printf("Usage;\n");
	printf("\t-kt ~~ Try getting kernel task\n");
	printf("\t-kb ~~ Find kernel base\n");
	printf("\t-kh ~~ Find the kernel header and print it to stdout\n");
	printf("\t-km ~~ Print kernel map\n");
    printf("\t-find ~~ Find bytes in kernel space, returns address\n");
    printf("\t\tamphet -find \"Darwin\"\n");
    printf("\t-hexdump [HEX_ADDRESS] [HEX_DUMP_SIZE] ~~ Hexdump memory at address\n\t\t*HEX_DUMP_SIZE is a optional size in hex, defaults to 0x100*\n");
    printf("\t\tamphet -hexdump 0x7fc3e49e 0x60\n");
    printf("\t-write [HEX_ADDRESS] [STRING TO WRITE] ~~ Write string to memory address\n");
    printf("\t\tamphet -write 0x7f2d1c5c \"Boolin\"\n");
}
...
char *available_patches[2] = {"IOSAESAccelerator uid key", "IOFlashControllerUserClient::externalMethod"};
void standard_find() {
    for (int i = 0; i < (sizeof(available_patches)/sizeof(*available_patches)); ++i)
    {
        printf("%d -- Search for %s\n", i, available_patches[i]);
    }
}

uint8_t *find_IOSAESAccelerator () {
    uint8_t* ptr = (uint8_t*) SHADOWMAP_BEGIN;
    
    vm_address_t occurance = 0;
    //unsigned char *str =  "\xB0\xF5\xFA\x6F\x00\xF0\x92\x80";
    if ((occurance = find_bytes_kern((vm_address_t)ptr, (vm_address_t)SHADOWMAP_END, (unsigned char *)"\xB0\xF5\xFA\x6F\x00\xF0\x92\x80", 8)) == 0) {
        return 0;
    }
    printf("Found IOSAESAccelerator enable uid key at 0x"ADDR"\n", occurance);
    return (uint8_t *)occurance;
    /*
...
int main(int argc, char **argv, char **envp) {
	printf("Amphet kernel utility v%.1f -- the best thing since sliced BUTTER\n", VERSION_NUMBER);
	printf("https://bugz4amphets.services for more info\n");
	task_t k_task = 0;
	...
	if (strcmp(argv[1], "-kt") == 0)
	{
		printf("Attempting to get kernel task\n");
		if (get_kernel_task(&k_task) != KERN_SUCCESS)
		{
			printf("Couldn't get kernel task.\n");
			return -1;
		} 
		printf("Got kernel task at 0x%08x\n", k_task);
        return 0;
	} else if (strcmp(argv[1], "-kb") == 0) {
		printf("Attempting to get kernel base\n");
		if (kernel_base == -1)
		{
			printf("Failed to get kernel base\n");
			return -1;
		}
		printf("Got kernel base at 0x"ADDR"\n", kernel_base);
        return 0;
	} else if (strcmp(argv[1], "-kh") == 0) {
		printf("Attempting to get kernel header\n");
		if (print_kernel_header() != 0)
		{
			printf("Some error occured! Exiting...\n");
			return -1;
		}
        return 0;
	} else if (strcmp(argv[1], "-km") == 0) {
		printf("Attempting to get kernel map\n");
		if (print_kernel_map() != 0)
		{
			printf("Some error occured! Exiting...\n");
			return -1;
		}
        ...
        printf("Searching for \"%s\"", argv[2]);
        if ((occurance = find_bytes_kern((vm_address_t)ptr, (vm_address_t)SHADOWMAP_END, (unsigned char *)argv[2], strlen(argv[2]))) == 0) {
            return 0;
        }
        printf("Found \"%s\" at 0x"ADDR"\n", argv[2], occurance);
        hexdump_kern_space((vm_address_t)occurance, 0x60);
        return -1;
       ...
       // uint8_t* ptr = (uint8_t*) SHADOWMAP_BEGIN;
        ...
        scanf("%i", &common_choice);
        printf("Finding %s\n", available_patches[common_choice]);
        switch (common_choice) {
            case 0:
                printf("Trying to get pointer now\n");
                ptr = find_IOSAESAccelerator();
                if (ptr == 0)
                {
                    break;
                }
                printf("Pointer at %p\n", ptr);
                hexdump_kern_space((vm_address_t)ptr, 0x100);
                return 0;
        ...
        /*
        printf("Searching for \"%s\" in kernel space\n", input_bytes);
		if ((start_addy = findStringInKernel("Darwin", kernel_task, kernel_base)) != 0)
		{
			printf("Found bytes at address ::: 0x"ADDR"\n", start_addy);
			hexdump_kern_space(start_addy, 0x69);
			wrote_bytes = write_kernel(start_addy, (unsigned char*)uname_patched, strlen(uname_patched)+1);
			if (wrote_bytes > 0)
			{
				printf("Successfully wrote "SIZE" bytes to kernel\n", wrote_bytes);
				hexdump_kern_space(start_addy, 0x69);
				return 0;
			}
			return 0;
        ...
        */
        printf("Failed to find in kernel space\n");
		return -1;
        ...
        vm_address_t address_a = (vm_address_t)strtol(argv[2], NULL, 16);
        size_t dump_size = 0;
        if (address_a == 0)
        {
            printf("Please specify an address to dump\n");
            return -1;
        }
        if (argv[3]) {
            dump_size = strtol(argv[3], NULL, 16);
            printf("Using custom size ::: 0x%lx\n", dump_size);
        } else {
            dump_size = 0x100;
        }
        printf("Printing hexdump of 0x"ADDR"\n", address_a);
        hexdump_kern_space(address_a, dump_size);
        return 0;
        ...
        vm_size_t wrote_bytes = write_kernel(address_a, (unsigned char*)argv[3], strlen(argv[3])+1);
        if (wrote_bytes > 0)
        {
            printf("Successfully wrote "SIZE" bytes to kernel\n", wrote_bytes);
            hexdump_kern_space(address_a, 0x69);
            return 0;
        }
        printf("Printing hexdump of 0x"ADDR"\n", address_a);
        hexdump_kern_space(address_a, dump_size);
